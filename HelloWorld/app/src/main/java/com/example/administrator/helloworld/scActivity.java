...
public class scActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.shoucang_view);

        sharedPreferences = getSharedPreferences("User", Context.MODE_PRIVATE);
        editor = sharedPreferences.edit();
        final String id = sharedPreferences.getString("id", null);

        mContext = scActivity.this;
        list_animal = (ListView) findViewById(R.id.list_shoucang);

        final SwipeRefreshLayout swipeContainer = (SwipeRefreshLayout) findViewById(R.id.sc_swipe_ly);
        swipeContainer.setOnRefreshListener(new SwipeRefreshLayout.OnRefreshListener() {
            @Override
            public void onRefresh() {
                //do all refreshing tasks
                HttpUrl.Builder urlBuilder = HttpUrl.parse("http://172.29.67.32:8080/CRUD/ListCollectByid").newBuilder();
                urlBuilder.addQueryParameter("collecter_id", id);
